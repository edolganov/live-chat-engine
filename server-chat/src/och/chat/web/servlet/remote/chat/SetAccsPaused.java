...
@WebServlet(URL_CHAT_PAUSED)
@SuppressWarnings("serial")
public class SetAccsPaused extends EncryptedJsonPostServlet<SetAccsPausedReq, Void> {
	
	@Override
	protected Void doJsonPost(HttpServletRequest req, HttpServletResponse resp, SetAccsPausedReq data) throws Throwable {
		
		pushToSecurityContext_SYSTEM_USER();
		try {
			
			for(String acc : data.accs){
				chats.setAccPaused(acc, data.val);				
			}
